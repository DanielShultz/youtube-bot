...
class YouTubeDownloader:
    ...
    def _extract_error(result: subprocess.CompletedProcess[str]) -> str:
        """Возвращает короткое понятное сообщение об ошибке yt-dlp."""
        ...
        if 'video unavailable' in lowered or 'this video is not available' in lowered:
            return 'Видео недоступно на YouTube: удалено, скрыто или ограничено.'
        if 'private video' in lowered:
            return 'Видео приватное и недоступно для загрузки.'
        if 'sign in to confirm your age' in lowered or 'age-restricted' in lowered:
            return 'Видео имеет возрастное ограничение. Нужны подходящие cookies.'
        if 'members-only' in lowered:
            return 'Видео доступно только подписчикам канала.'
        error_tail = raw_error[-500:] if raw_error else 'Неизвестная ошибка'
        return f'Ошибка yt-dlp: {error_tail}'
